...
from careermapblock.models import CountyStatValue, County, CountyStatType
import csv
import re
import os
...
def get_or_create_stat(county, column, val_str):
    stat_value = None
    try:
        stat_value = CountyStatValue.objects.get(
            county__name=county, stat_type__name=column)
    ...
    print "Setting value of stat %s to %s for county %s" % (
        column, val_str, county)
...
class Command(BaseCommand):
    ...
    2) the presence of a county_stats directory, with
       files in it whose name (e.g. under_5.csv) is the
       slug for a corresponding CountyStatType object.
    3) the format of each file to be:
        Moretooth County, 4
        Northwest County, 123
        Nerve County, 13
        South County, 155
    4) the existence of counties that correspond (case insensitive)
       to the county names in the file.
    """
    ...
    def handle(self, *args, **options):
        ...
        stat_types = dict([(slugify(c.name), c)
                           for c in CountyStatType.objects.all()])
        for filename in county_stat_columns:
            ...
            if stat_slug in stat_types.keys():
                ...
                var = raw_input(
                    (
                        "Replace values for column \"%s\" with the "
                        "values in file \"%s\" ? (y/n)\n"
                    ) % (
                ...
                print (
                    "Could not use this file: %s not found in "
                    "list of columns") % stat_slug
